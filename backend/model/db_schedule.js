...
async function addSchedule(outlet_name, start_time, end_time, vacancy) {
    ...
    const query = "INSERT INTO schedule (outlet_name, start_time, end_time, vacancy, employee_emails, employee_names) VALUES (?, ?, ?, ?, ?, ?)";
    try {
...
async function updateSchedule(schedule_id, outlet_name, start_time, end_time, vacancy, emails, names) {
    const query = "UPDATE schedule SET outlet_name = ?, start_time = ?, end_time = ?, vacancy = ?, employee_emails = ?, employee_names = ? WHERE schedule_id = ?";
    ...
    try {
        ...
        const [rows, fields] = await pool.execute(query, [outlet_name, start_time, end_time, vacancy, employee_emails, employee_names, schedule_id]);
        return rows;
    } catch (err) {
        throw new Error(err);
    }
...
async function addUserToSchedule(schedule_id, email, name) {
    try {
        ...
        const query = "UPDATE schedule SET employee_emails = ?, employee_names = ?, vacancy = ? WHERE schedule_id = ?";
        const [rows, fields] = await pool.execute(query, [emails_string, names_string, vacancy, schedule_id]);
        return rows;
    } catch (err) {
        throw new Error(err);
    }
