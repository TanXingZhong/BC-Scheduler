...
async function getWorkinghours(start_date, end_date) {
  try {
    const query = `SELECT u.id AS user_id, u.name, s.outlet_name, u.email, r.role_name AS role, u.phonenumber, u.bankName, u.bankAccountNo, COUNT(CASE WHEN s.start_time >= ? AND s.end_time <= ? AND s.published = TRUE THEN cs.schedule_id END) AS total_shifts,
    COALESCE(SUM(CASE WHEN s.start_time >= ? AND s.end_time <= ? AND s.published = TRUE THEN ABS(TIMESTAMPDIFF(MINUTE, s.start_time, IF(s.end_time < s.start_time, DATE_ADD(s.end_time, INTERVAL 1 DAY), s.end_time))) END), 0) AS total_minutes,
    COALESCE(SUM(CASE WHEN s.start_time >= ? AND s.end_time <= ? AND s.published = TRUE THEN ABS(TIMESTAMPDIFF(MINUTE, s.start_time, IF(s.end_time < s.start_time, DATE_ADD(s.end_time, INTERVAL 1 DAY), s.end_time))) END) DIV 60, 0) AS total_hours,
    COALESCE(SUM(CASE WHEN s.start_time >= ? AND s.end_time <= ? AND s.published = TRUE THEN ABS(TIMESTAMPDIFF(MINUTE, s.start_time, IF(s.end_time < s.start_time, DATE_ADD(s.end_time, INTERVAL 1 DAY), s.end_time))) END) % 60, 0) AS total_minutes 
